...
efficiency; a fully Bayesian version confirms that this inexpensive empirical-Bayes
pooling is already sufficient, as the binding constraint is over-dispersion rather
than parameter uncertainty. Finally, we model sighting \emph{type}: pooling cleanly
forecasts the
...
\begin{enumerate}
  ...
  a fully Bayesian comparison, that it is sufficient: propagating hyperparameter
  uncertainty does not improve forecasts in this over-dispersed regime.
...
falls. We instead slide the forecast origin forward one day at a time: at origin
$t$ the model is fit on observations $1{:}t$ and scored on $t{+}1{:}t{+}H$. Only
past data is ever used, and each CVE yields many scored forecasts, so comparisons
rest on a distribution of errors.
...
\begin{itemize}
  ...
  data (sightings/day);
  \item \textbf{quantile (pinball) loss} averaged over a grid of levels;
...
We backtest all models on five CVEs with sufficient history (origin-weighted, 2296
forecasts per model). \Cref{tab:tier1} and \Cref{fig:tier1} report the result.
...
\textbf{(ii) Negative Binomial beats Poisson} at every level, validating the
over-dispersion fix.
...
$c$ it uses only the \emph{other} CVEs' histories, mirroring the realistic setting
in which a new CVE arrives against an existing back-catalogue. We compare the
hierarchical hurdle against the best unpooled model from Experiment~1
(\texttt{indep\_hurdle\_nb}) and the rolling-mean baseline.
...
30 days (0.198), and at 7 days it matches the unpooled model at 30 days. Pooling
reaches in roughly a week what an isolated model does not reach in a month.
...
\subsection{Is full Bayesian inference worth it?}\label{sec:bayes}
...
the parameter uncertainty that a Bayesian treatment would remedy. We therefore
conclude that the inexpensive empirical-Bayes scheme is the right choice here, and
that the productive next step is a \emph{tighter likelihood}---modelling the
dependence between the activity and burst components, or a zero-inflated
formulation---rather than heavier inference. The Bayesian machinery remains useful
where one specifically wants calibrated credible intervals on the population
...
\begin{table}[H]
\centering
\caption{Total-count CRPS: typed decomposition versus single pooled model.}
\label{tab:decomp}
\begin{tabular}{lrrr}
\toprule
model & $W$=7 & 14 & 30 \\
...
effect and pools the activity and burst components independently. Notably, the
limitation one might expect to matter most---the empirical-Bayes use of point
hyperparameters---turns out not to: the fully Bayesian comparison of
\Cref{sec:bayes} shows that propagating hyperparameter uncertainty does not help,
because the binding problem is over-dispersion, not parameter uncertainty. The
clear implication is that the next gains lie in a \emph{tighter likelihood}. Two
concrete routes: a zero-inflated rather than hurdle formulation, and---in the
typed model, where the per-type streams are currently sampled independently---%
explicitly modelling the dependence between activity and burst (and between PoC
and exploitation, which do co-occur), which could sharpen the aggregate.
...
three findings that temper intuition: a fully Bayesian treatment does not beat the
cheap empirical-Bayes pooling (the limiting factor is over-dispersion, not
parameter uncertainty), decomposing sightings by type does not improve the
aggregate forecast, and open-source proof-of-concept activity does not cleanly
precede exploitation in this data. Each points the same way---toward tighter
likelihoods and richer dependence structure rather than heavier machinery. We
release all code, data, and figures to support reproducibility and further work.
