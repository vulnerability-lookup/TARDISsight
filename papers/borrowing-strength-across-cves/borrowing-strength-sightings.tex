...
\date{8 June 2026\\[0.6em]
  \small doi:\,\href{https://doi.org/10.5281/zenodo.22023078}%
  {10.5281/zenodo.22023078}}
...
training-window size, yields roughly a threefold improvement in data
efficiency, and continues to hold on a larger 200-CVE corpus; two robustness
checks---a fully Bayesian treatment and a zero-inflated
...
\begin{enumerate}
  ...
  the scarcity that defeated the first paper---and it replicates on a 200-CVE
  corpus, ruling out a small-sample artefact. We use empirical Bayes and show, via
...
models. A much larger 200-CVE corpus, used in \Cref{sec:scale} to confirm the
headline result is not an artefact of this curated set, is built automatically
from the most-sighted vulnerabilities.
...
unchanged (\Cref{tab:scale,fig:scale}): the pooled hierarchical hurdle has the lowest CRPS at
\emph{every} window, beating the unpooled hurdle and the rolling-mean baseline by
$16$--$35\%$ in CRPS, and PIT calibration remains competitive. The advantage of borrowing
...
\begin{table}[H]
\centering
\caption{Scale check --- CRPS by training-window size $W$ on the 200-CVE corpus
(2010--2026). Lower is better; compare \Cref{tab:pooling} (24 CVEs).}
\label{tab:scale}
\begin{tabular}{lrrrrrr}
\toprule
model & $W$=5 & 7 & 10 & 14 & 21 & 30 \\
\midrule
\textbf{hier\_hurdle} (pooled)   & \textbf{0.127} & \textbf{0.111} & \textbf{0.101} & \textbf{0.098} & \textbf{0.091} & \textbf{0.098} \\
indep\_hurdle\_nb (unpooled)     & 0.167 & 0.155 & 0.142 & 0.131 & 0.114 & 0.116 \\
rolling\_mean                    & 0.176 & 0.170 & 0.154 & 0.140 & 0.120 & 0.121 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\linewidth]{figures/scale_crps_vs_window.png}
\caption{CRPS versus training-window size on the 200-CVE corpus (mean $\pm$ SEM).
The pooled hierarchical hurdle keeps its lead over the unpooled hurdle and the
rolling-mean baseline at every window---the same picture as \Cref{fig:pooling}, now
with tighter bands from the larger sample.}
\label{fig:scale}
\end{figure}
