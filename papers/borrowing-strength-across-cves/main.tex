...
efficiency. Finally, we model sighting \emph{type}: pooling cleanly forecasts the
...
\begin{enumerate}
  ...
  the scarcity that defeated the first paper.
...
Our hierarchical model is an empirical-Bayes prototype: it uses point
hyperparameters and so does not propagate hyperparameter uncertainty, it shares a
single pooled burst dispersion rather than a per-CVE random effect, and it pools
the activity and burst components independently. A fully Bayesian multilevel
formulation \cite{gelman2007data} would address all three and likely close the
small residual mis-calibration at the shortest windows. In the typed model the
per-type streams are sampled independently; modelling their correlation (PoC and
exploitation do co-occur) is a natural next step that could improve the aggregate.
...
two findings that temper intuition: decomposing sightings by type does not improve
the aggregate forecast, and open-source proof-of-concept activity does not cleanly
precede exploitation in this data. We release all code, data, and figures to
support reproducibility and further work.
