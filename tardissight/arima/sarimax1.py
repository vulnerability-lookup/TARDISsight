...
future_dates = pd.date_range(
    start=daily_series.index[-1] + pd.Timedelta(days=1), periods=future_days
)
...
pred_mean.plot(label="Forecast (SARIMAX on log-transformed data without seasonal components)", color="orange")
plt.fill_between(
    conf_int.index,
    conf_int.iloc[:, 0],
    conf_int.iloc[:, 1],
